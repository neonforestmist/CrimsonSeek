...
function normalizePdfText(value: string) {
  return plainCitationText(value)
    .replace(/\u2018|\u2019/g, "'")
    .replace(/\u201c|\u201d/g, '"')
    .replace(/\u2013|\u2014/g, "-")
    .replace(/\u2026|\.{3,}/g, " ")
...
function hasValidCitation(content: string, evidenceItems: EvidenceItem[]) {
...
function sourceMaterialLine(evidenceItems: EvidenceItem[]) {
...
function weaveSavedSourceMaterial(content: string, moment: EvidenceSearchMoment | null) {
  void moment;
...
function sourceCardDomId(momentId: string, sourceIndex: number) {
  return `source-${momentId}-${sourceIndex + 1}`;
}

function scrollSourceCardIntoRail(momentId: string, sourceIndex: number) {
...
export function DebateArena() {
  ...
  const showComposer = messages.length > 0 && !auditMode;

  useEffect(() => {
    setComposerRoot(document.body);
  }, []);
  ...
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => {
      window.scrollTo({
        top: auditMode ? 0 : document.documentElement.scrollHeight,
  ...
  useEffect(() => {
    if (!evidenceRailOpen || activeSourceIndex === null || !activeSearchMomentId) return;

    let secondFrame = 0;
    const firstFrame = window.requestAnimationFrame(() => {
      secondFrame = window.requestAnimationFrame(() => {
        scrollSourceCardIntoRail(activeSearchMomentId, activeSourceIndex);
      });
    });
  ...
  }, [activeSearchMomentId, activeSourceIndex, evidenceRailOpen, sourceScrollRequest]);
  ...
  function loadAudit(arena: ArenaPreset) {
    ...
    const requestedMomentId = search.get("moment");
    const nextMoment =
      cleanMoments.find((moment) => moment.id === requestedMomentId) ?? cleanMoments[0] ?? null;
    const requestedSource = Number(search.get("source"));
    const requestedSourceIndex =
      Number.isInteger(requestedSource) && requestedSource > 0 ? requestedSource - 1 : null;
    const nextEvidence = nextMoment ? visibleEvidence(nextMoment.evidence) : [];
    const nextSourceIndex =
      requestedSourceIndex !== null && requestedSourceIndex < nextEvidence.length
        ? requestedSourceIndex
        : null;
    setSearchMoments(cleanMoments);
    setActiveSearchMomentId(nextMoment?.id ?? null);
    setActiveSourceIndex(nextSourceIndex);
    setEvidenceRailOpen(nextSourceIndex !== null);
    setEvidence(nextEvidence);
    setLastSearchQuery(nextMoment?.query ?? arena.prompt);
    setHasLiveEvidence(Boolean(nextEvidence.length));
    const momentsById = new Map(cleanMoments.map((moment) => [moment.id, moment]));
    setMessages(
  ...
  function selectSearchMoment(moment: EvidenceSearchMoment, sourceIndex: number | null = null) {
    ...
    if (hasSource) setSourceScrollRequest((request) => request + 1);
    ...
    if (auditMode) {
      ...
      params.set("audit", selectedArena.id);
      params.set("moment", moment.id);
      if (hasSource) {
        params.set("source", String((sourceIndex ?? 0) + 1));
      } else {
        params.delete("source");
      }
      window.history.replaceState(null, "", `?${params.toString()}#thread`);
    }
  }
  ...
  async function submitArgument() {
    ...
    try {
      ...
      const liveMoment = cleanSearchMoment({
        id: makeId("evidence"),
      ...
      const liveReply = weaveSavedSourceMaterial(data.reply, liveMoment);
      setMessages((current) => [
        ...current,
        {
          id: makeId("assistant"),
  ...
  return (
    <>
    <section
      className={`min-h-[calc(100svh-56px)] w-full max-w-full overflow-x-clip overflow-y-visible transition-[padding] duration-200 lg:min-h-[calc(100dvh-56px)] ${
        desktopStarterSidebarOpen ? "lg:pl-[320px] 2xl:pl-[340px]" : "lg:pl-[86px]"
      }`}
    >
      <div
        className={`grid min-h-[calc(100svh-56px)] w-full max-w-full min-w-0 items-start gap-0 overflow-x-clip p-3 sm:p-4 lg:min-h-[calc(100dvh-56px)] lg:p-5 ${
          showEvidenceRail
      ...
      >
        ...
        <main
          id="thread"
          className={`flex w-full max-w-full min-w-0 scroll-mt-24 flex-col overflow-hidden rounded-lg border border-divider/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.92),rgba(255,255,255,0.78))] shadow-[0_1px_2px_rgba(21,18,14,0.05),0_26px_62px_-38px_rgba(21,18,14,0.48)] ${
            showComposer
              ? "mb-[calc(6.5rem+env(safe-area-inset-bottom))] min-h-0 sm:mb-[calc(7rem+env(safe-area-inset-bottom))]"
              : "min-h-[calc(100svh-80px)] lg:min-h-[calc(100dvh-80px)]"
          } ${
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex min-w-0 items-start gap-3">
                  <div className="min-w-0">
                    ...
                    {activeArena.id !== CUSTOM_ARENA.id && (
                ...
                {messages.length > 0 && (
                  ...
                  >
                    ...
                    <span className="max-[390px]:sr-only">Export PDF</span>
                  </button>
                )}
              </div>
            </div>
            ...
            <div
              className={`min-h-0 max-w-full overflow-x-hidden bg-[radial-gradient(circle_at_50%_0%,rgba(217,79,48,0.08),transparent_35%)] px-4 py-4 sm:px-5 ${
                  ...
                  {messages.map((message) => {
                    ...
                    return (
                      <MessageBubble
                        ...
                        activeSourceIndex={
                          evidenceRailOpen && moment?.id === activeSearchMomentId ? activeSourceIndex : null
                        }
                    ...
                    <div className="flex items-start gap-2.5 sm:gap-3">
                      ...
                      <div className="min-w-0 w-full max-w-full flex-1 rounded-lg border border-divider/70 bg-canvas/68 px-3 py-3 text-[13px] text-ink-muted sm:px-4 sm:text-sm">
                        <span className="inline-flex min-w-0 flex-wrap items-center gap-2 font-semibold text-accent-700">
                          ...
                          Searching evidence
                          <MoreHorizontal className="h-4 w-4 flex-shrink-0 animate-pulse" />
                        </span>
        ...
        {mobileStarterSidebarOpen && (
          <>
            ...
            <StarterSidebar
              ...
              dialog
              onSelect={selectArena}
              className="fixed inset-y-0 left-0 z-[70] flex h-[100svh] max-h-[100svh] w-[min(88vw,360px)] pb-[calc(env(safe-area-inset-bottom)+0.75rem)] pt-[calc(env(safe-area-inset-top)+0.75rem)] lg:hidden"
            />
          </>
        )}
        ...
        {showEvidenceRail && (
          <>
            ...
            />
            <div
              className="fixed inset-y-0 right-0 z-[70] h-[100svh] max-h-[100svh] w-[min(92vw,390px)] p-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] pt-[calc(env(safe-area-inset-top)+0.75rem)] sm:p-4 xl:sticky xl:top-[76px] xl:z-auto xl:h-[calc(100dvh-96px)] xl:max-h-[calc(100dvh-96px)] xl:w-auto xl:p-0"
              role="dialog"
              aria-modal="true"
      ...
      {composerRoot && showComposer
        ? createPortal(
            <ComposerDock
              sidebarOpen={desktopStarterSidebarOpen}
              evidenceRailOpen={showEvidenceRail}
            >
              <Composer
                input={input}
                setInput={setInput}
...
function ComposerDock({
  ...
  return (
    <div
      className={`pointer-events-none fixed inset-x-0 bottom-0 z-50 transition-[padding] duration-200 ${
        sidebarOpen ? "lg:pl-[320px] 2xl:pl-[340px]" : "lg:pl-[86px]"
      } ${evidenceRailOpen ? "xl:pr-[340px] 2xl:pr-[360px]" : ""}`}
    >
      <div className="pointer-events-auto bg-canvas p-2.5 pb-[calc(env(safe-area-inset-bottom)+0.625rem)] shadow-[0_-1px_0_rgba(221,213,196,0.72)] sm:p-3 lg:px-5 lg:py-3">
        {children}
      </div>
    </div>
  );
...
function Composer({
  ...
  return (
    <form
      className="mx-auto w-full max-w-5xl rounded-[1.35rem] border border-divider/80 bg-white px-2.5 py-2 shadow-[0_1px_2px_rgba(21,18,14,0.06),0_-16px_34px_-30px_rgba(21,18,14,0.42)]"
      onSubmit={(event) => {
        event.preventDefault();
        submitArgument();
      }}
    >
      ...
      <div className="flex min-h-[3.25rem] items-end gap-2 rounded-[1rem] border border-transparent px-1 py-1 focus-within:border-accent-300 focus-within:shadow-[0_0_0_3px_rgba(217,79,48,0.12)]">
        <label htmlFor="debate-reply" className="sr-only">
          Your next point
        </label>
        <textarea
          id="debate-reply"
          ...
          className="min-h-10 max-h-36 flex-1 resize-none bg-transparent px-2 py-2.5 text-[16px] leading-6 text-ink placeholder:text-ink-faint focus:outline-none sm:max-h-40"
          aria-label="Write your next debate argument"
        />
        <button
          type="submit"
          disabled={!input.trim() || loading}
          className={`mb-0.5 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-500/25 ${
            input.trim() && !loading
              ? "bg-accent-500 text-white shadow-[0_1px_2px_rgba(150,47,24,0.16),0_10px_18px_-12px_rgba(217,79,48,0.8)] hover:bg-accent-600 active:scale-[0.98]"
              : "cursor-not-allowed bg-canvas-soft/78 text-ink-faint"
          }`}
...
function MessageBubble({
  ...
  return (
    <div className={`flex w-full min-w-0 max-w-full items-start gap-3 overflow-hidden ${isUser ? "flex-row-reverse" : ""}`}>
      <div
      ...
      <article
        className={`min-w-0 max-w-[calc(100%-3rem)] overflow-hidden rounded-lg border px-4 py-3 shadow-[0_1px_2px_rgba(21,18,14,0.04)] sm:max-w-[88%] ${
          isUser
      ...
      >
        ...
        <MessageContent
          ...
          activeSourceIndex={active ? activeSourceIndex : null}
...
function MessageContent({
  ...
  return (
    <div className="max-w-full overflow-hidden break-words text-sm leading-6 [overflow-wrap:anywhere]">
      <div className="space-y-3">
        {paragraphs.map((paragraph, index) => (
          <p key={`${paragraph.slice(0, 18)}-${index}`} className="whitespace-pre-wrap">
            {renderCitationSegments({
              text: paragraph,
              evidenceItems,
              searchMoment,
              onSelectSearchMoment,
...
function renderCitationSegments({
  text,
  evidenceItems,
  searchMoment,
  onSelectSearchMoment,
  ...
  for (const match of text.matchAll(pattern)) {
    ...
    if (searchMoment && item) {
      const material = sourceMaterial ? cleanSourceSnippet(sourceMaterial) : "";
      segments.push(
        <span key={`${matchIndex}-${number}`} className="citation-cluster">
          <span className="rounded-[0.28rem] bg-accent-50/72 px-1 py-0.5 font-semibold text-[color:color-mix(in_oklab,var(--color-accent-800)_78%,var(--color-ink)_22%)] ring-1 ring-accent-100/80">
            {material}
          </span>
          <button
            type="button"
            onClick={() => onSelectSearchMoment(searchMoment, sourceIndex)}
            title={`${item.title} - ${evidenceDomain(item.url)}`}
            className="mx-0.5 inline-flex h-[1.35rem] min-w-[1.55rem] translate-y-[-1px] items-center justify-center rounded-full border border-accent-200 bg-accent-50 px-1.5 text-[11px] font-extrabold leading-none text-accent-800 shadow-[0_1px_1px_rgba(150,47,24,0.08)] transition-colors hover:border-accent-300 hover:bg-white focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-500/25"
            aria-label={`Show source ${number}: ${item.title}`}
          >
            ({number})
          </button>
        </span>
...
function SourceReferenceRow({
  ...
  return (
    <div className="mt-4 flex flex-wrap items-center gap-1.5 text-[11px] font-semibold text-ink-faint">
      <span className="inline-flex h-6 items-center rounded-full border border-divider/70 bg-canvas/70 px-2.5 text-[11px] font-bold text-ink-muted">
        Sources
      </span>
      {numbers.map((number) => {
        ...
        const active = activeSourceIndex === number - 1;
        ...
        return (
          <button
            ...
            aria-pressed={active}
            className={`inline-flex h-6 min-w-8 items-center justify-center rounded-full border px-2.5 text-[11px] font-extrabold leading-none shadow-[0_1px_1px_rgba(150,47,24,0.08)] transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-500/25 ${
              active
                ? "border-accent-400 bg-accent-600 text-white"
                : "border-accent-200 bg-accent-50 text-accent-800 hover:border-accent-300 hover:bg-white"
            }`}
          >
            {number}
          </button>
...
function EvidenceRail({
  ...
  return (
    <aside id="evidence-rail" className="flex h-full min-h-0 flex-col overflow-hidden rounded-lg border border-divider/70 bg-white/90 p-4 shadow-[0_1px_2px_rgba(21,18,14,0.04),0_16px_42px_-30px_rgba(21,18,14,0.38)] xl:max-h-full xl:rounded-l-none xl:self-start xl:border-l">
      ...
      <div id="evidence-rail-scroll" className="min-h-0 flex-1 overflow-y-auto overscroll-contain pr-1">
      ...
      {evidence.length ? (
        <div className="space-y-3">
          ...
          <a
            ...
            id={
              activeSearchMoment
                ? sourceCardDomId(activeSearchMoment.id, index)
                : `source-${sourceNumber}`
            }
            href={item.url}
            target="_blank"
            ...
            className={`group block scroll-mt-4 rounded-lg border p-3 transition-all hover:border-accent-200 hover:bg-white ${
              active
                ? "border-accent-300 bg-accent-50/86 shadow-[0_0_0_3px_rgba(217,79,48,0.14),0_14px_32px_-26px_rgba(150,47,24,0.78)]"
                : "border-divider/70 bg-white/68"
            }`}
          >
            <div className="mb-2 flex items-center justify-between gap-3">
              <div className="inline-flex h-6 items-center rounded-full border border-accent-200 bg-white/80 px-2 text-[11px] font-extrabold text-accent-700">
                Source {sourceNumber}
              </div>
            ...
            <p className="mt-2 text-xs leading-5 text-ink-muted">{item.snippet}</p>
